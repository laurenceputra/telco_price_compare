...
for (let i = 0; i < phones.length; i++) {
    var name = phones[i].getElementsByClassName("device--name")[0].innerHTML.trim().replace(" <br>", "");
    var price = phones[i].getElementsByClassName("price--number")[0].innerHTML.trim();
    if (!phoneArr[name]) {
        phoneArr[name] = []
    }
